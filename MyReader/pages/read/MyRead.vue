<template>
  <view id="book-reader" style="height: 100%;">
    <view class="container container0">
      <!-- 章节名 -->
      <view class="chapter">
        {{chapterName}}
      </view>
      <view class="content">
        <view class="inner-box" :style="{height:`${innerHeight}px`}" @contextmenu.prevent="contextmenu"
          @touchstart="handleTouchStart" @touchend="handleTouchEnd" @longpress.prevent="contextmenu">
          <!-- <view ref="bookInnerRef" id="yourElementId" class="book-inner" v-html="chapterContent"
            :style="{fontSize:`${fontSize}px`,lineHeight: lineHeight}" selectable>
          </view> -->
          <rich-text class="book-inner" @selectionchange="handleSelectionChange" @contextmenu.prevent="contextmenu"
            v-html="chapterContent" :style="{fontSize:`${fontSize}px`,lineHeight: lineHeight}" :selectable="true">
          </rich-text>
        </view>

        <view class="contextmenu" v-show="showcontextmenu"
          :style="{ top:`${contextmenuTop}px`,left:`${contextmenuLeft}px`} ">
          <span @click="contextmenu_copy">复制</span>
          <span @click="contextmenu_note">笔记</span>
        </view>
        <view class="edit-note" v-show="showEditNote">
          <h1 style="text-align: center;">记笔记</h1>
          <div>{{ curNote.keywords }}</div>
          <div>
            <input v-model="curNote.note">
          </div>
          <div style="display: flex;background: rgb(245, 245, 245); padding: 15px 0;">
            <div style="flex: 1;margin: 0 5px;"><button size="default" type="default" plain="true"
                @click="showEditNote = false;">取消</button></div>
            <div style="flex: 1;margin: 0 5px;"><button size="default" type="primary" plain="true"
                @click="notes.push({...curNote});initPage();showEditNote = false;">确认</button></div>
          </div>
        </view>
      </view>
  ...
  export default {
    ...
    data() {
      return {
        ...
        bookId: 1,
        chapterId: 1,
        chapterName: '第一回 宴桃园豪杰三结义  斩黄巾英雄首立功',
        chapterText: `滚滚长江东逝水，浪花淘尽英雄。是非成败转头空。青山依旧在，几度夕阳红。    白发渔樵江渚上，惯看秋月春风。一壶浊酒喜相逢。古今多少事，都付笑谈中。
    ——调寄《临江仙》
...
    毕竟董卓性命如何，且听下文分解。`,
    ...
    methods: {
       ...
       **/
      async initPage() {
        this.chapterText = this.chapterText || '';
        this.notes = this.notes.sort((n1, n2) => n1.offset > n2.offset ? -1 : 1);
        console.log(this.notes);
        this.chapterContent = this.chapterText;
        for (let i = 0; i < this.notes.length; i++) {
          ...
          let leftText = this.chapterContent.substring(0, offset) || '';
          let rightText = this.chapterContent.substring(offset);

          let spanIndex = -1;
          spanIndex = rightText.indexOf('<span class=');
          if (spanIndex > -1 && spanIndex < length) {
            keywords = keywords.substring(0, spanIndex);
            length = keywords.length;
          }
          let keywordsHtml = `<span class="keywords" onclick="uni.showNote('${note}')">${keywords}</span>`;
          this.chapterContent = leftText + rightText.replace(keywords, keywordsHtml);
        }
        this.chapterContent = this.chapterContent.replace(/\n/g, '<br/>');
      ...
      calcHeight() {
        ...
        let lineNum = Math.floor((this.windowHeight - 60 + Math.floor((lineHeight - this.fontSize) / 2)) / lineHeight)
        this.innerHeight = lineNum * lineHeight
        console.log('linenume', lineNum);
      },
      ...
      handleTouchStart(event) {
        ...
        // 保存起始selection的offset
        this.showSelectionTag = true;
      },
      handleTouchEnd(event) {

      },
      handleSelectionChange(event) {
        console.log('选中的文本:', e.detail.range);
      },
      editNode(id, chapterId, bookId, offset, length, text) {
        this.showEditNote = true;
        this.curNote.id = id;
        this.curNote.chapterId = chapterId;
        this.curNote.offset = offset;
        this.curNote.length = length;
        this.curNote.keywords = text;
        this.curNote.note = '';
      },
      selectstart(event) {
      ...
      contextmenu(event) {
        ...
        this.contextmenuLeft = (event.clientX || event.changedTouches[0].clientX) - 20;
        this.contextmenuTop = (event.clientY || event.changedTouches[0].clientY) - 45 + 30;
        if (this.contextmenuLeft + 100 > this.windowWidth - 50) {
          this.contextmenuLeft = this.windowWidth - 150;
        }
        if (this.contextmenuTop + 30 > this.windowHeight - 60) {
          this.contextmenuTop = this.windowHeight - 90;
        }
      },
      contextmenu_copy() {
        if (window) {
      ...
      contextmenu_note() {
        if (window) {
          ...
          let offset = this.selectionOffset(selection);
          this.editNode(id, this.chapterId, this.bookId, offset, text.length, text);
          this.showcontextmenu = false;
        }
      },
      copy(text) {
      ...
      selectionOffset(selection) {
        ...
        while (dom.parentNode.nodeName != 'UNI-RICH-TEXT') {
          dom = dom.parentNode;
        }
